# This Python module was borrowed from GitHub user thatguysimon, with some alterations of my own (added code found on lines 45, 53-54, and 163-165. The original source code can be found at: <https://gist.github.com/thatguysimon/6caa622be083f97b8c5c9a10478ba058>.
# - Matt Diller
...
DATA_DIRECTORY = 'data/all-reports'
OUTPUT_DIRECTORY = 'data/standoff2corenlp-annotation-output'
CORENLP_SERVER_ADDRESS = 'http://localhost:9000'

NER_TRAINING_DATA_OUTPUT_PATH = join(OUTPUT_DIRECTORY, 'data.tsv')
RE_TRAINING_DATA_OUTPUT_PATH = join(OUTPUT_DIRECTORY, 'data.corp')
...
for file in ann_data_files:
    ...
    with open(join(DATA_DIRECTORY, file), 'r') as document_anno_file:
        date = file.split('.ann')[0].split('data_')[1]
    ...
    # write text and annotations into NER and RE standoff2corenlp-annotation-output files
        ...
        for sentence in output['sentences']:
            ...
            for sentence_row in sentence_re_rows:
                re_training_data.write(
                    '{}\t{}\t{}\tO\t{}\t{}\tO\tO\tO\n'.format(str(sentence_count), sentence_row['entity_type'],
                                                              str(token_count), sentence_row['pos_tag'],
                                                              sentence_row['word']))
                token_count += 1
            ...
            for relation in relations:
                if relation['standoff_entity1_id'] in entities_in_sentence and relation[
                    'standoff_entity2_id'] in entities_in_sentence:
...
dates_fname = 'data/nlp-results/report-ids'

with open(dates_fname, 'a') as f:
    if dates_lst:
        f.write('\n'.join(dates_lst) + '\n')